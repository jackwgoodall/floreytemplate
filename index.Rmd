...
This is a tutorial on the Rmarkdown template for Sheffield's [Florey Institute](https://www.sheffield.ac.uk/florey-institute) (although everyone is of course welcome to use and adapt it). 


This page has three aims:

  1. To give a demonstration of what the Florey Template finished product looks like
  2. To explain how to install, use and customise the template
  3. To provide a tutorial on how to use R Markdown for people who are familiar R but are new to markdown
  
For those of you who are new to markdown, I hope to convert you to this powerful communication tool, and this tutorial will demonstrate how markdown might be used to communicate pre-publication research findings with colleague and collaborators.  

This webpage was written with R Markdown using the Florey Template.  The code used to make it can be found on my GitHub [here](https://github.com/jackwgoodall/floreytemplate/blob/main/index.Rmd).  It might be useful to look at this in conjunction with this webpage to see how it has been put together, although this shouldn't be necessary to understand it.

If you are experienced already with markdown then the section ['installing the template'](#install-the-template) will be all you need to get the Florey branding and general template. However the [custom CSS](#custom-css) section later on might be of interest, as this explains how to customise the page, including how to customise the logo and section breaks.

If you are new to markdown then I suggest following this document through in order.


If you need more detail or want to understand how to use markdowjn to create PDFs, Word documents or Shiny webpages I would highly recommend  [the R Markdown Cookbook](https://bookdown.org/yihui/rmarkdown-cookbook/).
...
2. Then the install the template package with:  
...
These chunks are framed by the ```` ```{r}```` at the start and ` ``` ` at the end. 
